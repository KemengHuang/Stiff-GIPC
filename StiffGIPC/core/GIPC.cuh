...
{
  public:
    static constexpr std::size_t INITIAL_DCD_PAIR_CAPACITY = 100'000;
    static constexpr std::size_t INITIAL_CCD_PAIR_CAPACITY = 1'000'000;

    bool animation = false;
    double3* _vertexes = nullptr;  // borrowed: aliases device_TetraData::vertexes
    double3* _rest_vertexes = nullptr;  // borrowed: aliases device_TetraData::rest_vertexes
    cudatool::DeviceBuffer<uint3>    _faces;
    cudatool::DeviceBuffer<uint2>    _edges;
    cudatool::DeviceBuffer<uint32_t> _surfVerts;


    double3* targetVert = nullptr;  // borrowed: aliases device_TetraData::targetVert
    uint32_t* targetInd = nullptr;  // borrowed: aliases device_TetraData::targetIndex
    uint32_t softNum     = 0;
    uint32_t triangleNum = 0;


    double3* _moveDir = nullptr;  // borrowed: aliases pcg_data.dx
    lbvh_f   bvh_f;
    lbvh_e   bvh_e;

    PCG_Data pcg_data;

    cudatool::DeviceBuffer<int4>     _collisonPairs;
    cudatool::DeviceBuffer<int4>     _ccd_collisonPairs;
    cudatool::DeviceBuffer<uint32_t> _cpNum;
    cudatool::DeviceBuffer<int>      _MatIndex;
    cudatool::DeviceBuffer<uint32_t> _close_cpNum;

    cudatool::DeviceBuffer<uint32_t> _environment_collisionPair;

    cudatool::DeviceBuffer<uint32_t> _closeConstraintID;
    cudatool::DeviceBuffer<double>   _closeConstraintVal;

    cudatool::DeviceBuffer<int4>   _closeMConstraintID;
    cudatool::DeviceBuffer<double> _closeMConstraintVal;

    cudatool::DeviceBuffer<uint32_t> _gpNum;
    cudatool::DeviceBuffer<uint32_t> _close_gpNum;
    cudatool::DeviceBuffer<int>      _scalar_scratch;
    cudatool::DeviceBuffer<double2>  _distance_scratch;
    //uint32_t* _cpNum;
    ...
    cudatool::DeviceBuffer<double3> _groundNormal;
    cudatool::DeviceBuffer<double>  _groundOffset;

    // for friction
    cudatool::DeviceBuffer<double>                 lambda_lastH_scalar;
    cudatool::DeviceBuffer<double2>                distCoord;
    cudatool::DeviceBuffer<__GEIGEN__::Matrix3x2d> tanBasis;
    cudatool::DeviceBuffer<int4>                   _collisonPairs_lastH;
    uint32_t                    h_cpNum_last[5] = {0, 0, 0, 0, 0};
    cudatool::DeviceBuffer<int> _MatIndex_last;

    cudatool::DeviceBuffer<double>   lambda_lastH_scalar_gd;
    cudatool::DeviceBuffer<uint32_t> _collisonPairs_lastH_gd;
  ...
  public:
    ...
    double ground_largestFeasibleStepSize(double slackness);

    double self_largestFeasibleStepSize(double slackness, int numbers);

    double InjectiveStepSize(double slackness, double errorRate, uint4* tets);

    double cfl_largestSpeed();
