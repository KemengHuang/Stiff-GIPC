...
namespace math
{
    ...
    template <typename T>
    __host__ __device__ constexpr T abd(T& a)
    {
        return a > 0 ? a : -a;
    }

    template <typename T>
    __host__ __device__ constexpr void polar_decomposition(const Eigen::Matrix<T, 2, 2>& A,
    ...
    template <typename T>
    __host__ __device__ constexpr void qr_svd2x2(const Eigen::Matrix<T, 2, 2>& A,
    ...
    template <typename T>
    __host__ __device__ constexpr void flip_sign(int                     j,
    ...
    __host__ __device__ constexpr void sort_sigma(Eigen::Matrix<T, 3, 3>& U,
    ...
    template <int t, typename T>
    __host__ __device__ constexpr void process(Eigen::Matrix<T, 3, 3>& B,
                                               Eigen::Matrix<T, 3, 3>& U,
    ...
    template <typename T>
    __host__ __device__ constexpr void qr_svd(const Eigen::Matrix<T, 3, 3>& A,
                                              Eigen::Matrix<T, 3, 1>&       S,
                                              Eigen::Matrix<T, 3, 3>&       U,
                                              Eigen::Matrix<T, 3, 3>& V) noexcept
    {
